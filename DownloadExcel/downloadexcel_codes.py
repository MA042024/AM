...
def process_xml(xml_contents: list, df_template: pd.DataFrame, reversed_dict: dict) -> pd.DataFrame:
    filled_dfs = [
        fill_excel_from_xml_tree(xml_content, df_template.copy(), reversed_dict)
...
def main(xml_contents, unique_suffix):
    ...
    pickle_path = os.path.join(
        os.path.dirname(core_curate_app.__file__), "pythoncodes", "AM_excel_mapping.pkl"
    )
    ...
    filled_output_path = os.path.join(settings.BASE_DIR, f"Initial_Download_{unique_suffix}.xlsx")
    filled_df.to_excel(filled_output_path, sheet_name="Data")
    print(f"Process completed, saved filled Excel to '{filled_output_path}'")
    Final_path = os.path.join(settings.BASE_DIR, f"Download_Final_{unique_suffix}.xlsx")
