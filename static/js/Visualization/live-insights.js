...
document.addEventListener('DOMContentLoaded', function () {
    var state = { scope: 'all', workspaceIds: [], activeTest: null, initialized: false, wsMatches: [], wsHighlight: -1, correlationExpanded: false };
    var charts = {};
    var chartRegistry = {};
    var allWorkspaces = [];
    ...
    function loadTestDetail(testKey) {
        ...
        return fetch('/visu/live-insights/test/' + testKey + '/?' + scopeParams().toString(), { credentials: 'same-origin' })
            ...
            .catch(function () { toggle('li-test-loading', false); });
    }
    ...
    function renderNumericCard(containerId, summary, tooltipText) {
        ...
        el.innerHTML =
            '<div class="li-metric-header"><span class="li-metric-label">' + label + infoTooltip(tooltipText) + '</span></div>' +
            '<div class="li-metric-mean">' + mean.toFixed(2) + '</div>' +
    ...
    function renderOverview(data) {
        ...
        grid.appendChild(kpiTile(
            'Avg Binder Content',
            data.avg_binder_content != null ? data.avg_binder_content + '%' : '—',
            BARREL_ICON,
            data.avg_binder_content_n ? recordsLabel(data.avg_binder_content_n) : '',
            BINDER_CONTENT_TIP
        ));
        grid.appendChild(kpiTile(
            'Avg RAP Content',
            data.avg_rap_content != null ? data.avg_rap_content + '%' : '—',
            '<i class="fas fa-recycle"></i>',
            data.avg_rap_content_n ? recordsLabel(data.avg_rap_content_n) : '',
            RAP_CONTENT_TIP
        ));
        ...
        renderNumericCard('li-metric-rap', data.mixture.rap_content, RAP_CONTENT_TIP);
        renderNumericCard('li-metric-max-density', data.mixture.max_density, MAX_DENSITY_TIP);
        registerChart(
    ...
    function renderTestDetail(data) {
        ...
        renderNumericCard('li-metric-test-rap', data.mixture.rap_content, RAP_CONTENT_TIP);
        renderNumericCard('li-metric-test-max-density', data.mixture.max_density, MAX_DENSITY_TIP);
        registerChart(
        ...
        Object.keys(data.results || {}).forEach(function (key) {
            ...
            renderNumericCard(card.id, data.results[key], testTooltips[key]);
        });
            ...
            for (var col = 0; col < n; col++) {
                ...
                if (row === col) {
                    ...
                    if (!entry) {
                        ...
                        cell.style.background = correlationCellColor(entry.r, neutralRgb);
