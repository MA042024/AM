    ...
    getReportSections: function(data) {
        ...
        if (preds) {
            sections.push({
                lines: [
                    'Prediction Results',
                    `Marshall Stability: ${preds['S_pred_kN']} kN`,
                    { type: 'interval', symbol: 'sigma', sub: 'R', low: preds['S_sigma_kN-'], high: preds['S_sigma_kN+'], unit: 'kN' },
                    { type: 'interval', symbol: 'R', low: preds['S_R_kN-'], high: preds['S_R_kN+'], unit: 'kN' },
                    `Marshall Flow: ${preds['F_pred_mm']} mm`,
                    { type: 'interval', symbol: 'sigma', sub: 'R', low: preds['F_sigma_mm-'], high: preds['F_sigma_mm+'], unit: 'mm' },
                    { type: 'interval', symbol: 'R', low: preds['F_R_mm-'], high: preds['F_R_mm+'], unit: 'mm' }
                ],
                imageUrl: data.Mar_pred_url,
                imageWidth: 210,
                imageCenter: true
            });
        }
