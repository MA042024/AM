...
Sieve sizes backed by too few records are dropped from the band entirely in
`_sieve_band()`, rather than plotted. A mean/percentile computed from a
handful of records is sampling noise, not a real feature of the gradation
curve — left in, it shows up as an implausible dip or spike at one sieve size
flanked by much higher/lower neighbors that actually have enough data behind
them. "Too few" is relative, not fixed: a point needs at least
`SIEVE_SAMPLE_SIZE_RATIO` (80%) of that curve's own best-reported size's
sample count, so the cutoff self-scales with however much data is in the
current scope instead of a fixed count that becomes too lenient as the
dataset grows or too strict on a small filtered scope.
`MIN_SIEVE_SAMPLE_SIZE_FLOOR` (50) is an absolute backstop underneath that
ratio, since 80% of a tiny max (e.g. 10 records) would still let a genuinely
noisy point through. If a curve's best-reported sieve size has fewer than
this many records, every point is dropped and the whole band comes back
empty (see "no data" handling above) rather than showing anything. Dropping the point is preferred over dropping the
underlying records: the records themselves aren't bad data, they just don't
happen to report a value at that particular sieve size.
...
`Visualization/live_insights_fields.py` declares, per test, which XSD
...
- `Visualization/live_insights_fields.py` — XSD-derived field/path config.
- `Visualization/live_insights_service.py` — scope resolution, XML
  parsing (reuses `downloadexcel_codes.extract_paths_and_values`), and
  aggregation (counts, mean + 95% interval summaries, sieve band).
- `Visualization/toolsviews.py` — `live_insights_workspaces`,
  `live_insights_tests`, `live_insights_overview`, `live_insights_test_detail`.
- `Visualization/tools_urls.py` — routes under `visu/live-insights/`.
