...
from DownloadExcel.downloadexcel_codes import extract_paths_and_values
...
# Sieve sizes that only a handful of records happen to report produce a
# mean/percentile band that's just sampling noise, not a real feature of the
# gradation curve -- visually this shows up as an implausible zigzag (e.g. a
# dip at one size flanked by much higher neighbors). A point is only kept if
# its sample size is at least SIEVE_SAMPLE_SIZE_RATIO of the curve's own
# best-reported size (self-scaling as the dataset grows, rather than a fixed
# count that becomes too lenient at scale or too strict on a small filtered
# scope), with MIN_SIEVE_SAMPLE_SIZE_FLOOR as an absolute backstop so a tiny
# max (e.g. 10, in a narrow filtered scope) can't let an 80%-of-that-max
# point of 8 records through.
SIEVE_SAMPLE_SIZE_RATIO = 0.8
MIN_SIEVE_SAMPLE_SIZE_FLOOR = 50
...
def _mix_type_value(parsed):
    return parsed.values.get(fields.MIXTURE_TYPE_FIELD[0])


def _year_value(parsed):
    raw = parsed.values.get(fields.YEAR_FIELD[0])
    if not raw:
        return None
...
def _binder_grade_value(parsed):
    path = fields.MIXTURE_TOP_N_FIELDS["target_binder_grade"][0]
    return parsed.values.get(path)


def get_workspace_mix_types(workspace_ids, user):
    workspace_ids = [w for w in (workspace_ids or []) if w]
    if not workspace_ids:
        return []
    parsed_records = _iter_parsed_records(
        _resolve_records("workspace", workspace_ids, user)
    )
    values = {_mix_type_value(p) for p in parsed_records}
    return sorted(v for v in values if v)


def get_workspace_years(workspace_ids, user):
    workspace_ids = [w for w in (workspace_ids or []) if w]
    if not workspace_ids:
        return []
    parsed_records = _iter_parsed_records(
    ...
    values = {_year_value(p) for p in parsed_records}
    return sorted((v for v in values if v), reverse=True)


def get_workspace_binder_grades(workspace_ids, user):
    workspace_ids = [w for w in (workspace_ids or []) if w]
    if not workspace_ids:
        return []
    parsed_records = _iter_parsed_records(
    ...
    values = {_binder_grade_value(p) for p in parsed_records}
    return sorted(v for v in values if v)


def _filter_by_mix_types(parsed_records, mix_types):
    mix_types = [m for m in (mix_types or []) if m]
    if not mix_types:
        return parsed_records
    allowed = set(mix_types)
    return [p for p in parsed_records if _mix_type_value(p) in allowed]


def _filter_by_years(parsed_records, years):
    years = [y for y in (years or []) if y]
    if not years:
        return parsed_records
    allowed = set(years)
    return [p for p in parsed_records if _year_value(p) in allowed]


def _filter_by_binder_grades(parsed_records, binder_grades):
    binder_grades = [b for b in (binder_grades or []) if b]
    if not binder_grades:
        return parsed_records
    allowed = set(binder_grades)
    return [p for p in parsed_records if _binder_grade_value(p) in allowed]
...
def _year_trend(parsed_records):
    years = [y for p in parsed_records if (y := _year_value(p))]
...
def _sieve_band(size_to_values, label):
    max_n = max((len(values) for values in size_to_values.values()), default=0)
    threshold = max(MIN_SIEVE_SAMPLE_SIZE_FLOOR, round(max_n * SIEVE_SAMPLE_SIZE_RATIO))
    band = []
    for size in sorted(size_to_values):
        summary = _numeric_summary(size_to_values[size], None)
        if summary["n"] < threshold:
            continue
...
def get_overview(scope, workspace_ids, user, mix_types=None, years=None, binder_grades=None):
    parsed_records = list(_iter_parsed_records(_resolve_records(scope, workspace_ids, user)))
    parsed_records = _filter_by_mix_types(parsed_records, mix_types)
    parsed_records = _filter_by_years(parsed_records, years)
    parsed_records = _filter_by_binder_grades(parsed_records, binder_grades)
...
def get_test_detail(test_key, scope, workspace_ids, user, mix_types=None, years=None, binder_grades=None):
...
def get_correlation_pair(test_key, field_a, field_b, scope, workspace_ids, user, mix_types=None, years=None, binder_grades=None):
