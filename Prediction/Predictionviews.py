...
from .VolumetricProperties import predict_volumetric_outputs
...
@require_http_methods(["POST"])
def predict_volumetric(request):
    try:
        session_id = request.POST.get('session_id')
        if not session_id:
            return JsonResponse({
        ...
        rap_content = float(request.POST.get('RAPContent', 0))
        binder_content = float(request.POST.get('BinderContent', 0))
        max_density = float(request.POST.get('MaximumDensity', 0))
        sieve_data = {
            '0.063': float(request.POST.get('sieve_0_063', 0)),
        ...
        out = predict_volumetric_outputs(
