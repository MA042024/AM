...
def live_insights_overview(request):
    ...
    if denied:
        return denied
    try:
        return JsonResponse(li.get_overview(scope, workspace_ids, request.user))
...
def live_insights_test_detail(request, test_key):
    ...
    if denied:
        return denied
    try:
        return JsonResponse(li.get_test_detail(test_key, scope, workspace_ids, request.user))
    except li.LiveInsightsError as e:
...
def live_insights_correlation_pair(request, test_key, field_a, field_b):
    ...
    if denied:
        return denied
    try:
        return JsonResponse(li.get_correlation_pair(test_key, field_a, field_b, scope, workspace_ids, request.user))
    except li.LiveInsightsError as e:
        status = 403 if 'access' in str(e).lower() else 400
        return JsonResponse({'error': str(e)}, status=status)
